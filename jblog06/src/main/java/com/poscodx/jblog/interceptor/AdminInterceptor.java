...
public class AdminInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		/* 정규 표현식 패턴
		   'http://localhost:8080/jblog06/'와 그 다음 슬래시('/') 사이 [blogId] 세그먼트 추출
			[^/] : '/' 문자를 제외(^ 부정을 의미)한 모든 문자와 매치한다는 의미
			+ : 바로 앞의 패턴([^/]+)이 하나 이상의 문자를 나타내야 한다는 의미
			즉, '/'가 아닌 문자가 하나 이상 나오는 경우를 추출
			jblog06/와 / 사이에 오는 blogId를 추출할 수 있다
			
			Ex) requestURI : http://localhost:8080/jblog05/yizi/admin/basic
			=> blogId : yizi
		*/
        Pattern pattern = Pattern.compile("/jblog06/([^/]+)/");
